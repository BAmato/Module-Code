...
struct Node {
    ...
    int edgeCount;          // Counter to track the number of edges
};
...
// Helper function to add edges, ensuring not to exceed the maximum allowed edges
void addEdge(int from, int to, int weight, const char* name) {
    if (nodes[from].edgeCount < MAX_EDGES) {
        nodes[from].edges[nodes[from].edgeCount++] = {to, weight, name};
        nodes[to].edges[nodes[to].edgeCount++] = {from, weight, name}; // Since it's undirected
    } else {
        Serial.print("Max edges exceeded for node ");
        Serial.println(from);
    }
}
...
void dijkstraInit() {
    ...
    for (int i = 0; i < MAX_VERTICES; i++) {
        ...
        nodes[i].edgeCount = 0; // Initialize edge count
    }

    // Define edges here (edges represent streets connecting two POI (vertex))
    addEdge(sect1, warehouse, 5, "US-375");
    addEdge(sect1, utep, 1, "Dead End");
    addEdge(sect1, sect2, 2, "US-54");
    addEdge(sect1, sect6, 3, "I-10");
    addEdge(sect2, airport, 1, "Dead End");
    addEdge(sect2, sect3, 2, "Spur");
    addEdge(sect3, alamo, 1, "Dead End");
    addEdge(sect3, ftbliss, 1, "Dead End");
    addEdge(sect3, sect4, transmountain, "Transmountain");
    addEdge(sect4, lascruces, deadend, "Las Cruces");
    addEdge(sect4, outlets, deadend, "Dead End");
    addEdge(sect4, sect5, doniphan, "Doniphan");
    addEdge(sect5, countryclub, deadend, "Dead End");
    addEdge(sect5, franklin, deadend, "Dead End");
    addEdge(sect5, sect6, mesa, "Mesa");
    addEdge(sect6, sunland, deadend, "Dead End");
    addEdge(sect6, executive, deadend, "Dead End");
    addEdge(sect6, sect1, i10, "I-10");

    Serial.println("------------Vertexes and Edges Added-------------");

    // Print detailed node and edge information
    for (int i = 0; i < MAX_VERTICES; i++) {
        Serial.print("Node ");
        Serial.print(i);
        Serial.print(" (");
        ...
        Serial.print(") has ");
        Serial.print(nodes[i].edgeCount);
        Serial.println(" edges:");
        for (int j = 0; j < nodes[i].edgeCount; j++) {
            Serial.print(" -> Edge to ");
            Serial.print(vertexNames[nodes[i].edges[j].to]);
            Serial.print(" with weight ");
            Serial.print(nodes[i].edges[j].weight);
            Serial.print(" (");
...
void dijkstra(int src) {
    ...
    for (int i = 0; i < MAX_VERTICES - 1; i++) {
        ...
        for (int j = 0; j < nodes[u].edgeCount; j++) {
            int v = nodes[u].edges[j].to;
            int weight = nodes[u].edges[j].weight;
    ...
    // Optional: Output distances for debugging
    Serial.println("Distances from source:");
    for (int i = 0; i < MAX_VERTICES; i++) {
        Serial.print("Vertex ");
        Serial.print(i);
        Serial.print(": ");
